...
import React, { useEffect, useState } from 'react';
import { Button, Dialog, DialogTitle, DialogContent, DialogContentText, DialogActions, TextField, Card, FormControl, InputLabel, Select, MenuItem, Chip, Box } from '@mui/material';
import { useNavigate } from 'react-router-dom';
...
export default function CreateAccount() {
    ...
    const handleCreateAccount = async () => {
        ...
        console.log('Creating account with:', { firstName, lastName });
        // Example fetch request (update as needed)
        try {
            const response = await fetch('http://localhost:8080/users', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ firstName, lastName /* other fields */ }),
            });
            ...
            displayDialogMessage('Error', error.message);
        }
    };

    // Function to display dialog messages
    const displayDialogMessage = (title, message) => {
        setDialogTitle(title);
        setDialogMessage(message);
        setDialogOpen(true);
